# -*- coding: utf-8 -*-
"""
Created on Tue Feb  1 01:04:56 2022
...
# rename rho to rho
    ...
    case = 'finalRunCvary\\28_Mar';#'mathCheck\\2_Feb'#'reversionAttempt4\\14_Nov';#'muNN-l2b\\2_Nov';#'baseline\\31_Oct';
else:
    sig_vec=[0];
    s_sig_vec = ['\\0\\'];
...
if use_muQ:
                        ...
                        for ww, elb in enumerate(rho_vec):
                            ...
                            for vv, el in enumerate(sig_vec):
                                ...
                                if showNonReprogQ:
                                    ...
                                    axCellNo[0,0].plot(data1[nn][tt][uu][ss][ww][vv][lll_load][0,:st2_stop]+EOT_diff,data1[nn][tt][uu][ss][ww][vv][lll_load][1,:st2_stop] * total_cell_num,'--',color=COL1.get_rgb(nn),label='CSC '+s_lgd_1+s_sig+'_'+str(lll_load))
                ...
                for uu, elu in enumerate(xi2_vec):
                    ...
                    for ww, elb in enumerate(rho_vec):
                        ...
                        for vv, el in enumerate(sig_vec):
                            ...
                            st2_stop = np.flatnonzero(data2[nn][tt][uu][ss][ww][vv][lll_load][0] < T_stop).max();
                            axRest[1,0].plot(data2[nn][tt][uu][ss][ww][vv][lll_load][0,:st2_stop],xi3* data2[nn][tt][uu][ss][ww][vv][lll_load][1,:st2_stop]/(1+xi3* data2[nn][tt][uu][ss][ww][vv][lll_load][1,:st2_stop]),'--',color=COL2.get_rgb(nn),label='feedforward rate'+s_lgd_0+s_sig+'_'+str(lll_load))
                            axRest[1,0].set_ylabel("feedforward rate"+log_str)
                            axRest[1,0].set_xlabel("time (days)")
                            axRest[1,0].set_title(s_title)
                            if showNonReprogQ:
                                st1_stop = np.flatnonzero(data1[nn][tt][uu][ss][ww][vv][lll_load][0] < T_stop).max();
                                axRest[1,0].plot(data1[nn][tt][uu][ss][ww][vv][lll_load][0,:st1_stop]+EOT_diff,xi3 * data1[nn][tt][uu][ss][ww][vv][lll_load][1,:st1_stop]/(1+ xi3*data1[nn][tt][uu][ss][ww][vv][lll_load][1,:st1_stop]),'--',color=COL1.get_rgb(nn),label='feedforward rate'+s_lgd_1+s_sig+'_'+str(lll_load))
        
        st0_stop = np.flatnonzero(dataN[nn][tt][uu][ss][ww][vv][lll_load][0] < T_stop).max();
        axRest[1,0].plot(dataN[nn][tt][uu][ss][ww][vv][lll_load][0,:st0_stop],xi3 * dataN[nn][tt][uu][ss][ww][vv][lll_load][1,:st0_stop]/(1+xi3*dataN[nn][tt][uu][ss][ww][vv][lll_load][1,:st0_stop]),'g--',label='feedforward rate, no treatment_'+str(lll_load))
                ...
                for uu, elu in enumerate(xi2_vec):
                    ...
                    for ww, elb in enumerate(rho_vec):
                        ...
                        for vv, el in enumerate(sig_vec):
                            ...
                            st2_stop = np.flatnonzero(data2[nn][tt][uu][ss][ww][vv][lll_load][0] < T_stop).max();
                            #
                            axRest[1,1].plot(data2[nn][tt][uu][ss][ww][vv][lll_load][0,:st2_stop],r2/(1+h2*data2[nn][tt][uu][ss][ww][vv][lll_load][2,:st2_stop]) * xi3*data2[nn][tt][uu][ss][ww][vv][lll_load][1,:st2_stop]/(1+xi3*data2[nn][tt][uu][ss][ww][vv][lll_load][1,:st2_stop]) - d-(1.1*r2-d)*hd*data2[nn][tt][uu][ss][ww][vv][lll_load][2,:st2_stop]/(1+hd*data2[nn][tt][uu][ss][ww][vv][lll_load][2,:st2_stop]),'--',color=COL2.get_rgb(nn),label='effective division rate'+s_lgd_0+s_sig+'_'+str(lll_load))
                            axRest[1,1].set_ylabel("effective division rate"+log_str)
                            axRest[1,1].set_xlabel("time (days)")
                            axRest[1,1].set_title(s_title)
                            if showNonReprogQ:
                                st1_stop = np.flatnonzero(data1[nn][tt][uu][ss][ww][vv][lll_load][0] < T_stop).max();
                                axRest[1,1].plot(data1[nn][tt][uu][ss][ww][vv][lll_load][0,:st1_stop]+EOT_diff,r2/(1+h2*data1[nn][tt][uu][ss][ww][vv][lll_load][2,:st1_stop]) * xi3*data1[nn][tt][uu][ss][ww][vv][lll_load][1,:st1_stop]/(1+xi3*data1[nn][tt][uu][ss][ww][vv][lll_load][1,:st1_stop]) - d-(1.1*r2-d)*hd*data1[nn][tt][uu][ss][ww][vv][lll_load][2,:st1_stop]/(1+hd*data1[nn][tt][uu][ss][ww][vv][lll_load][2,:st1_stop]),'--',color=COL1.get_rgb(nn),label='effective division rate'+s_lgd_1+s_sig+'_'+str(lll_load))
        
        st0_stop = np.flatnonzero(dataN[nn][tt][uu][ss][ww][vv][lll_load][0] < T_stop).max();
        axRest[1,1].plot(dataN[nn][tt][uu][ss][ww][vv][lll_load][0,:st0_stop],r2/(1+h2*dataN[nn][tt][uu][ss][ww][vv][lll_load][2,:st0_stop]) * xi3*dataN[nn][tt][uu][ss][ww][vv][lll_load][1,:st0_stop]/(1+xi3*dataN[nn][tt][uu][ss][ww][vv][lll_load][1,:st0_stop]) - d-(1.1*r2-d)*hd*dataN[nn][tt][uu][ss][ww][vv][lll_load][2,:st0_stop]/(1+hd*dataN[nn][tt][uu][ss][ww][vv][lll_load][2,:st0_stop]),'g--',label='effective division rate, no treatment_'+str(lll_load))
    ...
    for lll_load in load_temp:
        for vv, el in enumerate(sig_vec):
            ...
            # if log_yscaleQ:
            #     axT.set_yscale('log')
            axRest[0,0].plot(data2[vv][lll_load][0,:st2_stop],data2[vv][lll_load][3,:st2_stop],'--',color=COL2.get_rgb(lll_load),label='mu '+s_lgd_0+s_sig+'_'+str(lll_load))
            # axRest[0,0].plot(data1[vv][lll_load][0,:st1_stop],data1[vv][lll_load][1,:st1_stop] * total_cell_num,color[1]+'--',label='CSC '+s_lgd_1)
            # axRest[0,0].plot(data1[vv][lll_load][0,:st1_stop],data1[vv][lll_load][2,:st1_stop] * total_cell_num,color[1]+'-',label='DCC '+s_lgd_1)
            # axT.plot(dataN[lll_load][0,:st0_stop],dataN[lll_load][1,:st0_stop] * total_cell_num,color[2]+'--',label='CSC, no treatment')
            # axT.plot(dataN[lll_load][0,:st0_stop],dataN[lll_load][2,:st0_stop] * total_cell_num,color[2]+'-',label='DCC, no treatment')
            axRest[0,0].legend(fancybox=True, framealpha=0.5);
            axRest[0,0].set_ylabel("mu"+log_str)
            axRest[0,0].set_xlabel("time (days)")
            axRest[0,0].set_title(s_title)
            if saveQ:
                figTs.savefig(plot_drty+"pop"+plot_save_suffix+".png",dpi=300)
    ...
    for lll_load in load_temp:
        for vv, el in enumerate(sig_vec):
            ...
            if log_yscaleQ:
                axCellNo[1,0].set_yscale('log')
            axCellNo[1,0].plot(data2[vv][lll_load][0,:st2_stop],(data2[vv][lll_load][1,:st2_stop]+data2[vv][lll_load][2,:st2_stop]) * total_cell_num,'--',color=COL2.get_rgb(lll_load),label='total '+s_lgd_0+s_sig+'_'+str(lll_load))
            axCellNo[1,0].plot(data1[vv][lll_load][0,:st1_stop],(data1[vv][lll_load][1,:st1_stop]+data1[vv][lll_load][2,:st1_stop]) * total_cell_num,color[1]+'--',label='total '+s_lgd_1)
            #axCellNo[1,0].plot(dataN[lll_load][0,:st0_stop],(dataN[lll_load][1,:st0_stop]+dataN[lll_load][2,:st0_stop]) * total_cell_num,color[2]+'--',label='total, no treatment')
            axCellNo[1,0].legend(fancybox=True, framealpha=0.5);
            axCellNo[1,0].set_ylabel("Total Cell Number"+log_str)
            axCellNo[1,0].set_xlabel("time (days)")
            axCellNo[1,0].set_title(s_title)
    ...
    # TODO: maybe add the other ones too?
    fig, axC = plt.subplots(figsize=(8,8));
    for lll_load in load_temp:
        for vv, el in enumerate(sig_vec):
            s_sig = '('+ str(el) +')';
            s_title = "(l:"+f"{float(l_vec[lll_load]):.3}"+";h1:"+f"{float(h1_vec[lll_load]):.3}"+";h2:"+f"{float(h2_vec[lll_load]):.3}"+";hd:"+f"{hd:.3}"+s_title_tweakable+")";
            ...
            if log_yscaleQ:
                axCellNo[1,1].set_yscale('log')
            axCellNo[1,1].plot(data2[vv][lll_load][0,:st2_stop],csc_p[0][:st2_stop],'-',color=COL2.get_rgb(lll_load),label='CSC '+s_lgd_0+s_sig+'_'+str(lll_load))
            axCellNo[1,1].plot(data1[vv][lll_load][0,:st1_stop],csc_p[1][:st1_stop],color[1]+'-',label='CSC '+s_lgd_1)
            #axCellNo[1,1].plot(dataN[lll_load][0,:st0_stop],csc_p[2][:st0_stop],color[2]+'-',label='CSC, no treatment')
            axCellNo[1,1].legend(fancybox=True, framealpha=0.5);
            axCellNo[1,1].set_ylabel("CSC frac")
            axCellNo[1,1].set_xlabel("time (days)")
            axCellNo[1,1].set_title(s_title)
