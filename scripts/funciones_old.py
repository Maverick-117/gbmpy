...
def dU_dt_mk0(U,t, r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, chi):
    # uses pre-d(mu)/dt modification
...
def dU_dt_mki(U,t, r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, beta):
    #function dU = stem_ODE_feedback(t, U, r1, r2, d, p, h, hd, z, l, n, sig, mu_bar, chi)
    return np.array([(2*p/(1+l*U[1]**n)-1)*r1/(1+h1*U[1]**n)*U[0], 
                     2*(1-p/(1+l*U[1]**n))*r1/(1+h1*U[1]**n)*U[0] + U[1] * (r2/(1+h2*U[1]**n) - d-(1.1*r2-d)*hd*U[1]**n/(1+hd*U[1]**n)),
                    sig * (mu_bar - U[2])
                    ])
    # make simulations, argue from equations

def dU_dt(U,t, r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, beta, xi1, xi2, ddd):
    #function dU = stem_ODE_feedback(t, U, r1, r2, d, p, h, hd, z, l, n, sig, mu_bar, chi)
    return np.array([(2*p/(1+l*U[1]**n)-1)*r1/(1+h1*U[1]**n)*U[0] + beta * xi1*ddd/(1+xi1*ddd) * U[2] * U[1], #- 0*(d+(1.1*r2-d)*hd*U[1]**n/(1+hd*U[1]**n)) * U[0], 
                      2*(1-p/(1+l*U[1]**n))*r1/(1+h1*U[1]**n)*U[0] + U[1] * (r2/(1+h2*U[1]**n) - d-(1.1*r2-d)*hd*U[1]**n/(1+hd*U[1]**n) - beta * xi1*ddd/(1+xi1*ddd) * U[2]),
...
def radiotherapy_mk0(U, LQ_para, surv_vec):
    def fdbk(control, surv):
        val = 1/(1+control*surv);
        return val
    u, v, s = U[:,-1];
    ...
    #v_new = max(0,(SF_V - min(1,c*D))*v); # apply RT; assumes death occurs on non-reprogrammed cells. is this biologically valid?
    v_new = max(0,(1 - min(1,c*D))*SF_V*v);
    #u_new = u*SF_U + min(1,c*D*fdbk(cont_c, s))*v_new;
    u_new = u*SF_U + min(1,c*D*fdbk(cont_c, s))*SF_V*v;
    # max(v*exp(-a2*D-b2*D^2) - c*v*D,0)
    s_new = s + srvn_csc * (u-u*SF_U) + srvn_dcc * (v-v_new); 
    # v_new is used instead of SF_U because radiotherapy causes de-dif
    return [u_new,v_new, s_new,SF_U, SF_V]

def radiotherapy_mki(U, LQ_para, surv_vec):
    u, v, mu = U[:,-1];
    [a1, b1, a2, b2, c, D] = LQ_para;
    [cont_p_a, cont_p_b, compt_mult, srvn_csc, srvn_dcc, cont_c] = surv_vec;
    # compt_mult tunes fold-change of stem cell feedback ratio based on diff.
...
def radiotherapy_mkii(U, LQ_para, surv_vec):
    #radiotherapy_mkii
    u, v, mu = U[:,-1];
    [a1, b1, a2, b2, c, D] = LQ_para;
    [cont_p_a, cont_p_b, compt_mult, srvn_csc, srvn_dcc, cont_c] = surv_vec;
...
def dynamics_mk0(para_values, sim_values):
    #using the non-survivin/reprogramming amount-based dynamical moddel
    ...
    if model1Q or model2Q:
        U[2,:] = np.multiply(U[2,:], np.exp(-sig * (T))); # using an integrating factor to bypass stiffness
    ...
    ####### with stem cell #########
        ...
        if model1Q or model2Q:
            U_new[2,:] = np.multiply(U_new[2,:],np.exp(-sig * (T_int-treat_days[i]))); # using an integrating factor to bypass stiffness
        U = np.hstack((U, U_new))
...
def dynamics_mki(para_values, sim_values):
    #dynamics_mki
    ...
    for t_idx, t in enumerate(T[:-1]):
        U_new = integrate.odeint(dU_dt, np.array(U0_t).reshape((3,)), T[t_idx:t_idx+2], args=(r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, chi)).T
        u_upd = U_new[:,-1];
        u_upd += np.array([min(1,u_upd[2])*u_upd[1],-min(1,u_upd[2])*u_upd[1],0]);
        U = np.hstack((U,u_upd.T.reshape((3,1))))
        U0_t = u_upd;
    ...
    ####### with stem cell #########
        ...
        U = np.hstack((U,np.array([U0_int]).T))
        for t_idx, t in enumerate(T_int[:-1]):
            U_new = integrate.odeint(dU_dt, np.array(U0_int).reshape((3,)), T_int[t_idx:t_idx+2], args=(r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, chi)).T

            u_upd = U_new[:,-1];
            if i ==  len(sim_resume_days):
                u_upd += np.array([min(1,U_new[2,-1])*U_new[1,-1],-min(1,U_new[2,-1])*U_new[1,-1],0]);
            U = np.hstack((U,u_upd.T.reshape((3,1))))
            U0_int = u_upd;
    ...
    T_none = np.linspace(0, treat_days[-1],time_pts2)
    U0_t = U0;
    U_none = np.array([U0]).T;
    for t_idx, t in enumerate(T_none[:-1]):
        #print(np.array(U0_t))
        U_new = integrate.odeint(dU_dt, np.array(U0_t).reshape((3,)), T_none[t_idx:t_idx+2], args=(r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, chi)).T
        U_none = np.hstack((U_none,U_new[:,-1].reshape((3,1))));
        U0_t = U_new[:,-1];
    
    return U, T, U_none, T_none

def dynamics_mki_alt(para_values, sim_values):
    model0Q, model1Q, model2Q, kimReprogQ, total_cell_num, treat_days, mu_start, LQ_param, total_start_frac, sc_start, sim_resume_days, surv_vec, time_pts1, time_pts2 = sim_values;
    r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, chi = para_values;
    tc_start = total_start_frac-sc_start;
    #print("data loaded")
...
def dynamics_mki_alt2(para_values, sim_values):
    model0Q, model1Q, model2Q, kimReprogQ, total_cell_num, treat_days, mu_start, LQ_param, total_start_frac, sc_start, sim_resume_days, surv_vec, time_pts1, time_pts2 = sim_values;
    r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, chi = para_values;
    tc_start = total_start_frac-sc_start;
    #print("data loaded")
    ...
    ## radiotherapy dynamics 
    for i in range(len(sim_resume_days)-1):
    ...
    ####### with stem cell #########
        ...
        T = np.concatenate((T, T_int))
    #print("done")
    T_fin = np.linspace(sim_resume_days[-1], treat_days[-1],int(np.round(time_pts2*(-sim_resume_days[-1] + treat_days[-1]))));
    
    for t_idx, t in enumerate(T_fin[:-1]):
        U_new = integrate.odeint(dU_dt, np.array(U0_int).reshape((3,)), T_fin[t_idx:t_idx+2], args=(r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, chi)).T
        # if model1Q or model2Q:
        #     U_new[2,:] = np.multiply(U_new[2,:],np.exp(-sig * (T_int-treat_days[i]))); # using an integrating factor to bypass stiffness
...
def dynamics_mkii(para_values, sim_values):
    # dynamics_mkii
    model0Q, model1Q, model2Q, kimReprogQ, total_cell_num, treat_days, mu_start, LQ_param, total_start_frac, sc_start, sim_resume_days, surv_vec, time_pts1, time_pts2 = sim_values;
    r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, beta = para_values;
    tc_start = total_start_frac-sc_start;
    # ODE simulation before first fraction of RT
    # With treatment 
    U0 = [sc_start, tc_start, mu_start]; 
    ...
    U0_t = U0;
    for t_idx, t in enumerate(T[:-1]):
        U_new = integrate.odeint(dU_dt, np.array(U0_t).reshape((3,)), T[t_idx:t_idx+2], args=(r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, beta)).T
        u_upd = U_new[:,-1];
        #u_upd += np.array([min(1,u_upd[2])*u_upd[1],-min(1,u_upd[2])*u_upd[1],0]);
        U = np.hstack((U,u_upd.T.reshape((3,1))))
        U0_t = u_upd;
    ...
    for i in range(len(sim_resume_days)):
        ...
        U = np.hstack((U,np.array([U0_int]).T))
        for t_idx, t in enumerate(T_int[:-1]):
            U_new = integrate.odeint(dU_dt, np.array(U0_int).reshape((3,)), T_int[t_idx:t_idx+2], args=(r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, beta)).T
    ...
    for t_idx, t in enumerate(T_none[:-1]):
        #print(np.array(U0_t))
        U_new = integrate.odeint(dU_dt, np.array(U0_t).reshape((3,)), T_none[t_idx:t_idx+2], args=(r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, beta)).T
        U_none = np.hstack((U_none,U_new[:,-1].reshape((3,1))));
        U0_t = U_new[:,-1];
...
def parameter_setup(switch_vec, misc_pars):
    ...
    if compDosesQ:
        ...
        Doses = sorted(np.arange(1,21,dtype=float).tolist() + [40/15,34/10]);
