...
def dU_dt(U,t, r1, r2, d, p, h1, h2, hd, z, l, n, sig, mu_bar, rho, xi1, xi2, ddd, xi3, dsc, hU):
    ...
    dvdt=2*(1-p/(1+l*U[1]**n))*r1/(1+h1*U[1]**n)*U[0]/(1+hU*U[0]) + U[1] * (r2/(1+h2*U[1]**n) * xi3*U[0]/(1+xi3*U[0]) - d-(1.1*r2-d)*hd*U[1]**n/(1+hd*U[1]**n));#-rho* xi1*ddd/(1+xi1*ddd) * U[2])
...
def parameter_setup(switch_vec, misc_pars):
    ...
    if compDosesQ:
        ...
        Doses = [2.0];#np.arange(1,22,dtype=float);#np.arange(2,22,2,dtype=float);#sorted(np.arange(1,21,dtype=float).tolist() + [40/15,34/10]);
        Frac = [30.0];#list(map(lambda d: float(np.floor(calc_BED_Frac(.17,.02,d))),Doses)); # 25*(1+(5)/8.5)
